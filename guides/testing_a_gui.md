...
painted. They are fast enough to belong in ./scripts/bazel run //:verify: the handful that exist run in
well under a second.
...
- ./scripts/bazel run //:verify accepts whatever the tests paint: it runs them with UPDATE_SNAPSHOTS=1, so
  ...
  painting to the pull request's branch as a commit. Everywhere else CI runs ./scripts/bazel run //:verify -- --check, which sets nothing,
...
- A changed painting is for a person to review, not for you. They review it in a Paint
  ...
  at all.
- ./scripts/bazel run //crates/paint-snapshot:rasterize-example -- snapshots/<crate>.<name>.paint out.png
...
- The exception is a painting you cannot account for: if you do not know why one changed,
  ...
  snapshots/ && ./scripts/bazel run //:verify -- --check --plugin-tests - and the failure says which frame
...
./scripts/bazel run //:verify runs them, through Bazel: ./scripts/bazel run //:verify -- --plugin-tests is the plugin
tests alone, and accepts what they paint. Bazel compiles each plugin's tests for
...
While working on one editor, run its tests alone:
  ...
  ./scripts/bazel test //crates/editors/checklist:test
  ./scripts/bazel test //crates/editors/checklist:test --test_env=UPDATE_SNAPSHOTS=1

The first compares, the second accepts. A single test is a filter handed through to the
test binary: ./scripts/bazel test //crates/editors/checklist:test --test_arg=some_test_name.
