...
Keyboard regression tests run without a window. `./scripts/bazel test //crates/beui:test` runs beui's tests, and `./scripts/bazel run //:verify` runs the whole workspace's.
