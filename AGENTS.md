...
Verification:
- `./scripts/bazel`: Bazel, which builds, lints and tests the workspace on BuildBuddy's remote workers; every command below is a Bazel target. It installs the pinned Bazel into the checkout the first time it runs. It needs a BuildBuddy API key: `BUILDBUDDY_API_KEY` in the environment, or the key in `.buildbuddy-api-key` at the root of the checkout or in `~/.config/be3/buildbuddy-api-key`. See guides/bazel.md for what it covers. A module extension (`bazel/cargo/extension.bzl`) reads cargo's own plans for the workspace whenever a Cargo.toml or Cargo.lock changes, makes the third-party crates from them, and a crate's `BUILD.bazel` file takes its dependencies from what it recorded, so Cargo.toml is the only place a dependency is declared. What it made is recorded in `MODULE.bazel.lock`, which is checked in: commit it with the Cargo.toml change.
- `./scripts/bazel run //crates/block-app:app`: builds the native app with every plugin beside it and runs it.
- `./scripts/bazel run //:check`: Use this for fast compile feedback. It runs rustc's check pass over every first-party target through Bazel - the host's and the plugins' and games' wasm - and fails with the compiler's errors. Prefer this over cargo, which the workspace no longer builds with.
- `./scripts/bazel run //:verify`: The full check. It applies every autofix, accepts new and changed snapshots, and runs all lints and tests. CI does the same on a pull request and pushes whatever it changes to the pull request's branch as a commit. Use a 10 minute timeout in the tool call arguments so it is less likely to convert itself to a background task.
  - A plugin's own tests are compiled to wasm and run through the plugin host on this machine, because they read and write the accepted paintings in `snapshots/`; `./scripts/bazel run //:verify -- --plugin-tests` runs them. `block-editor-plugin` is in that run too, because the guest half of the plugin framework only exists on wasm. For faster feedback on one editor, run `./scripts/bazel test //crates/editors/checklist:test`, adding `--test_env=UPDATE_SNAPSHOTS=1` to accept its paintings.
...
- `./scripts/bazel run //crates/block-app:android`: run this for changes that affect features specific to Android. It builds the APK and signs it into `target/android/block-app.apk`; `-- --install` installs it with adb and starts it.
- `./scripts/bazel build //crates/block-app:web`: run this for changes that affect features specific to web. `./scripts/bazel run //crates/block-app:web-serve` serves it, with be-server behind it, on http://127.0.0.1:8080.
- `./scripts/bazel run //crates/block-app:smoke`: run this for changes that could affect native startup or runtime integration. It performs a bounded automated launch in a virtual display with isolated data.
