...
- `./scripts/verify`: This is the primary full check. It is not required before committing: on a pull request, CI runs the same fixes, and when they change anything (including a new or changed snapshot) it fails that run and pushes the fixes to the pull request's branch as a commit, so pull that commit before pushing again. Run it locally when you want the test results or the fixes sooner, and use a 10 minute timeout in the tool call arguments so it is less likely to convert itself to a background task.
  ...
  - A plugin's own tests are not part of the workspace run: they are compiled to wasm and run through the plugin host by `scripts/internal/test-plugins.sh`, which verify calls for you. `block-editor-plugin` is in that run too, because the guest half of the plugin framework only exists on wasm. Run that script on its own for faster feedback on one editor (`scripts/internal/test-plugins.sh -p checklist`); it takes `cargo nextest run`'s arguments.
