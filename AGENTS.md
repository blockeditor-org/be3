...
- `./scripts/verify`: Run this after coherent changes and before committing, with a 10 minute timeout in the tool call arguments so it doesn't convert itself to a background task.
  - It applies clippy and formatting lints, autofixing what it can. It does not run the test suite: CI runs the full suite (`./scripts/verify --check`) on every push and pull request, so a local run no longer needs to run every test. Run `cargo nextest run` or `scripts/internal/test-plugins.sh` directly if you want to run tests locally, e.g. for a narrow diagnostic after a failure CI reported.
  - A plugin's own tests are not part of the workspace run: they are compiled to wasm and run through the plugin host by `scripts/internal/test-plugins.sh`. Run that script on its own for faster feedback on one editor (`scripts/internal/test-plugins.sh -p checklist`); it takes `cargo nextest run`'s arguments.
