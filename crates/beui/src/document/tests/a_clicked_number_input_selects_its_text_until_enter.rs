...
fn a_clicked_number_input_selects_its_text_until_enter() {
    ...
    assert_eq!(unstyled::text_input_selection(harness.document(), field), [0..4]);

    let left = pos2(harness.rect(field).left() + 12.0, middle.y);
    harness.drag(left, left + Vec2::new(18.0, 0.0));
    harness.frame(Vec::new());

    assert!(changes.borrow().is_empty(), "dragging inside the field must not change the value");
    ...
    assert!(number_input_text(harness.document(), input).is_some(), "enter closes the field");
}
