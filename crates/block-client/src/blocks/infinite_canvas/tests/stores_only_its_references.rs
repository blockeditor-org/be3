...
fn stores_only_its_references() {
    let mut block = InfiniteCanvas::new();
    assert!(block.references().is_empty());
    assert_eq!(serde_json::to_string(&block).unwrap(), "{\"references\":[]}");

    let linked = Uuid::new_v4();
    let operation = InfiniteCanvas::bridged_references(vec![linked]).expect("references are bridged");
    InfiniteCanvas::apply_operation(&mut block, &operation);
