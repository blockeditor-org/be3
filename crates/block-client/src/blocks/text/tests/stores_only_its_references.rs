...
fn stores_only_its_references() {
    let mut block = TextDocument::new();
    assert!(block.references().is_empty());
    assert_eq!(serde_json::to_string(&block).unwrap(), "{\"references\":[]}");

    let linked = Uuid::new_v4();
    let operation = TextDocument::bridged_references(vec![linked]).expect("references are bridged");
    TextDocument::apply_operation(&mut block, &operation);
