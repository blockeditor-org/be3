use block::Block;
use serde::{Deserialize, Serialize};
use uuid::Uuid;

pub use be_block::database::{DatabaseColor, DatabaseRow, DatabaseValue};

#[derive(Clone, Debug, Default, Deserialize, Eq, PartialEq, Serialize)]
pub struct Database {
    references: Vec<Uuid>,
}

impl Database {
    pub fn new() -> Self {
        Self::default()
    }

    pub fn with_references(references: Vec<Uuid>) -> Self {
        Self { references }
    }
}

#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(tag = "operation", rename_all = "snake_case")]
pub enum DatabaseOperation {
    SetReferences { references: Vec<Uuid> },
}

impl Block for Database {
    type Operation = DatabaseOperation;
    type History = block::NoHistory;

    const TYPE_ID: Uuid = Uuid::from_u128(0x0064_6174_6162_6173_652d_626c_6f63_6b01);
    const CRDT: bool = true;

    fn apply_operation(block: &mut Self, operation: &Self::Operation) {
        match operation {
            DatabaseOperation::SetReferences { references } => {
                block.references.clone_from(references);
            }
        }
    }

    fn references(&self) -> Vec<Uuid> {
        self.references.clone()
    }

    fn bridged_references(references: Vec<Uuid>) -> Option<Self::Operation> {
        Some(DatabaseOperation::SetReferences { references })
    }
}

#[cfg(test)]
mod tests;
