...
pub(crate) fn world_to_clip(position: [f32; 2], frame: &RenderFrame) -> [f32; 2] {
    [
...
#[cfg(test)]
pub(crate) fn frame_triangles(frame: &RenderFrame) -> Vec<DrawTriangle> {
    let mut triangles = background_triangles(frame);
    triangles.extend_from_slice(&frame.triangles);
    triangles
}

pub(crate) fn background_triangles(frame: &RenderFrame) -> Vec<DrawTriangle> {
...
pub(crate) fn wire_vertices(wires: &[DrawWire], frame: &RenderFrame) -> Vec<WireVertex> {
...
pub(crate) fn ray_vertices(rays: &[DrawRay], frame: &RenderFrame) -> Vec<WireVertex> {
    let half_width = frame.viewport_size[0] / frame.zoom * 0.5;
    let half_height = frame.viewport_size[1] / frame.zoom * 0.5;
...
pub(crate) fn stub_vertices(stubs: &[DrawStub], frame: &RenderFrame) -> Vec<WireVertex> {
...
pub(crate) fn value_triangle_vertices(
...
pub(crate) fn add_axis_lines(triangles: &mut Vec<DrawTriangle>, bounds: [f32; 4], width: f32) {
    let [left, top, right, bottom] = bounds;
