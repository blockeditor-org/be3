use super::support::{
    TestServer, access_for, add_member, create, create_workspace, list_access, register, set_access,
};
...
async fn block_access_survives_a_server_restart() {
    ...
    let server = TestServer::start_at_as(root, owner.token.clone(), owner.id, workspace.id).await;
