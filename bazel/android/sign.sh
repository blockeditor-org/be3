#!/bin/sh
#
# Signs an APK Bazel built, here, because the key is this machine's: Android
...
#
# Usage:
#   ./scripts/bazel run //crates/block-app:android -- [--out APK] [--keystore FILE] [--install]
set -eu
jdk="$(cd "$1" && pwd)" build_tools="$(cd "$2" && pwd)" platform_tools="$(cd "$3" && pwd)"
unsigned="$(cd "$(dirname "$4")" && pwd)/$(basename "$4")"
shift 4
# Bazel runs this from its runfiles; what it writes goes in the checkout.
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        *) echo "Usage: ./scripts/bazel run //crates/block-app:android -- [--out APK] [--keystore FILE] [--install]" >&2; exit 1 ;;
    esac
done
