...
triple="$(buck2_triple)"
sha256="$(buck2_release_sha256 starlark_fmt "$triple")"

release="starlark_fmt-$triple$(buck2_release_suffix)"
...
zstd --decompress --force --quiet "$archive" -o "$bin_directory/starlark_fmt$(buck2_binary_suffix)"
chmod +x "$bin_directory/starlark_fmt$(buck2_binary_suffix)"
rm "$archive"

echo "Installed starlark_fmt at $bin_directory/starlark_fmt$(buck2_binary_suffix)"
