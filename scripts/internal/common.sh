...
# The shared build cache: a bazel-remote speaking the remote execution API,
# holding the outputs of actions CI has already run. Nothing runs on it; buck2
# only asks whether an action's result is already known. guides/buck2.md says
...
#
# Everything about the host toolchain now reaches buck2 through the wrappers in
# buck/tools rather than through this file, so what is left here is the shared
# cache: an address, and whether this machine has the credential for it.
buck_config_local() {
    echo '# @generated by ./scripts/buck for this machine. Do not check this in.'
    buck_cache_config_local
}

# The shared build cache, when this machine has the credential for it.
#
# buck2 expands $NAME in http_headers itself, so what is written here names the
# environment variable rather than holding the secret - the file is generated
# and ignored by git either way, but a header that is only ever a variable is
# one that cannot be pasted somewhere it should not be.
#
# Without the credential nothing is written and the execution platform in
# buck/platforms/BUCK stays local-only. That is the case a fresh checkout is in,
# and it has to build rather than fail against a server it cannot talk to.
#
# BLOCKS_CACHE_UPLOAD is separate, and CI's. Reading someone else's cached
# compiler output is a bet on the cache being honest; writing to it is what
...
[buck2_re_client]
action_cache_address = $buck_cache_address
cas_address = $buck_cache_address
...
# that a machine that is already new enough is not sent somewhere else. The
# prelude's own helpers run on this, and one of them uses
# Path.relative_to(walk_up=True), which arrived in 3.12; an older one fails
# from inside an unrelated C compile, as a TypeError nobody would connect to it.
buck_python() {
    ...
    echo 'buck2 needs a Python of at least 3.12 and none was found on PATH.' >&2
    return 1
}
...
# The wrappers in buck/tools, one per host tool buck2 runs.
#
# Two problems are solved by the same file. buck2 writes an artifact into a
# command line as a path relative to the repository root, so a wrapper is the
# same string on every machine where an absolute path is not - which is what
# lets one machine use what another cached. And the version line makes the
# wrapper's bytes depend on the tool it runs, so a machine on a different
# compiler gets different cache keys rather than silently taking results
# compiled by something else.
#
# Both rest on the wrapper naming the tool rather than pointing at it: the name
# is resolved on PATH when the wrapper runs, which is per-machine, while the
# bytes are not.
buck_tool_wrapper() {
...
# Prefers the name with the version in it, so that two machines that resolve
# the same tool write the same wrapper even when one of them reaches it through
# a differently named symlink.
buck_versioned_command() {
    local plain="$1" versioned="$2"
...
# Writes buck/tools, and only the wrappers whose contents would change: buck2
# reads them as sources, so rewriting one with the same bytes still costs a
# rebuild of everything that ran through it.
write_buck_tools() {
    ...
    if [[ -z "$compiler" ]]; then
        echo 'No C compiler was found on PATH. Install clang.' >&2
        return 1
    fi
    local compiler_version major
    compiler_version="$("$compiler" --version 2>&1 | head -1)"
    major="$(echo "$compiler_version" | sed -n 's/.*version \([0-9]*\).*/\1/p')"
    write_buck_tool "$directory/cc" \
        "$(buck_versioned_command "$(basename "$compiler")" "clang-$major")" "$compiler_version"

    local cxx_compiler
    cxx_compiler="$(command -v clang++ || command -v c++)"
    if [[ -z "$cxx_compiler" ]]; then
        echo 'No C++ compiler was found on PATH. Install clang.' >&2
        return 1
    fi
    write_buck_tool "$directory/cxx" \
        "$(buck_versioned_command "$(basename "$cxx_compiler")" "clang++-$major")" \
        "$("$cxx_compiler" --version 2>&1 | head -1)"

    local archiver
    archiver="$(command -v llvm-ar || command -v ar)"
    if [[ -z "$archiver" ]]; then
        echo 'No archiver was found on PATH. Install binutils or llvm.' >&2
        return 1
    fi
...
# Writes .buckconfig.local, and only when its contents would change: buck2
