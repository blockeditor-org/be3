...
# The buck2 and reindeer this repository is built with.
#
# Both are pinned by commit rather than by release, because buck2 publishes its
# binaries on GitHub releases and reindeer publishes none at all, and building
# from a commit is the one route that is the same on every machine and in CI.
# A buck2 binary carries the prelude it was built with, so this commit pins the
# build rules too, which is why it is the one number that has to move for a
# prelude change to arrive.
buck2_revision='39ec8f3b4ae62ee9f7f530f8b1ee41dbbeb74c03'
reindeer_revision='3be7bd2e11d8fe949ae6db0bcbc46db8adab7264'

# buck2 does not build on stable: its own rust-toolchain asks for this nightly,
# and so does reindeer's. Nothing in this workspace is compiled with it.
buck2_toolchain='nightly-2026-07-05'

# What .buckconfig.local has to say for buck2 to work on this machine.
#
# .buckconfig names both of these with a plain command name as its default,
# which is what a machine whose python3 is new enough and whose clang is on
# PATH would resolve anyway. The default is not enough for buck2 because the
# prelude exec's the C compiler by path rather than searching for it, and
# because a python3 older than 3.12 fails from inside an unrelated C compile
# with a TypeError nobody would connect to this.
buck_config_local() {
    local python
    ...
    cat << EOF
# @generated by ./scripts/buck for this machine. Do not check this in.
[python]
interpreter = $python
...
# that a machine that is already new enough is not sent somewhere else.
buck_python() {
    local candidate
